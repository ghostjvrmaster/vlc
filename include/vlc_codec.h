...
#   define VLCDEC_RELOAD    (-100)
    ...
    /* Misc output callbacks */
    int             (*pf_misc_out_format_update)( decoder_t * );
    ...
    /* XXX use decoder_QueueMisc */
    int             (*pf_queue_misc)( decoder_t *, block_t * );
...
/**
 * This function queues a single miscellaneous, decoded data stream block for output.
 *
 * \note
 * The caller doesn't own the data anymore after this call (even in case of error).
 *
 * \return 0 if the block is queued, -1 on error
 */
static inline int decoder_QueueMisc( decoder_t *dec, block_t *p_block )
{
    assert( p_block->p_next == NULL );
    assert( dec->pf_queue_misc != NULL );
    return dec->pf_queue_misc( dec, p_block );
}
...
VLC_USED
static inline int decoder_UpdateMiscFormat( decoder_t *dec )
{
    assert(dec->fmt_in.i_cat == DATA_ES);
    if(dec->fmt_in.i_cat == DATA_ES && dec->pf_misc_out_format_update != NULL) {
        return dec->pf_misc_out_format_update(dec);
    } else {
        return -1;
    }
}
