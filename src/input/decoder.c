...
/*****************************************************************************
* Modified 01/15/2019
*  - Ryan Loebs <ryanl@jauntvr.com>
*
* -Added DATA_ES support
*****************************************************************************/
...
static int misc_out_update_format(decoder_t *p_dec)
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;

    if(p_owner->p_misc_out == NULL) {
        misc_output_t *misc_out = input_resource_RequestMiscOut(p_owner->p_resource);

        vlc_mutex_lock(&p_owner->lock);

        p_owner->p_misc_out = misc_out;
        DecoderUpdateFormatLocked(p_dec);

        vlc_mutex_unlock(&p_owner->lock);

        if(misc_out == NULL) {
            msg_Err(p_dec, "failed to create misc output");
            return -1;
        }
    }
...
static void DecoderPlayMisc( decoder_t *p_dec, block_t *p_misc, unsigned *restrict pi_lost_sum )
{
    decoder_owner_sys_t *p_owner = p_dec->p_owner;
    misc_output_t *p_misc_out = p_owner->p_misc_out;

    assert(p_misc != NULL);

    vlc_mutex_lock(&p_owner->lock);
    // write to owner here if necessary
    vlc_mutex_unlock(&p_owner->lock);

    if(p_misc->i_pts <= VLC_TS_INVALID) {
        msg_Warn(p_dec, "non-dated misc buffer received");
        *pi_lost_sum += 1;
        block_Release(p_misc);
        return;
    }
    ...
    DecoderWaitUnblock(p_dec);
    DecoderFixTs(p_dec, &p_misc->i_pts, NULL, NULL, &i_rate, DECODER_BOGUS_VIDEO_DELAY);
    vlc_mutex_unlock(&p_owner->lock);


    if(p_misc_out != NULL) {
        p_misc_out->present(p_misc_out, p_misc);
    } else {
        msg_Dbg(p_dec, "discard misc data buffer");
        *pi_lost_sum += 1;
        block_Release(p_misc);
    }
}

static void DecoderUpdateStatMisc( decoder_owner_sys_t *p_owner,
		                   unsigned decoded, unsigned lost )
{
    (void) p_owner; (void) decoded; (void) lost;
}

static int DecoderQueueMisc( decoder_t *p_dec, block_t *p_misc )
{
    unsigned lost = 0;
    decoder_owner_sys_t *p_owner = p_dec->p_owner;

    DecoderPlayMisc( p_dec, p_misc, &lost );
                                  ...
                                  sout_instance_t *p_sout )
{
    ...
    p_dec->pf_misc_out_format_update = misc_out_update_format;
    ...
    switch( p_dec->fmt_out.i_cat )
    {
        ...
        case SPU_ES:
            ...
            break;
        case DATA_ES:
            p_dec->pf_queue_misc = DecoderQueueMisc;
            p_owner->pf_update_stat = DecoderUpdateStatMisc;
            break;
        default:
...
static void DeleteDecoder( decoder_t * p_dec )
{
    ...
    if(p_owner->p_misc_out)
    {
        input_resource_PutMiscOut(p_owner->p_resource, p_owner->p_misc_out);

        if(p_owner->p_input != NULL) {
            input_SendEventMiscOut(p_owner->p_input);
        }
    }
