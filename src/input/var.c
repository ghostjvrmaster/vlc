...
static int EsSpuCallback ( vlc_object_t *p_this, char const *psz_cmd,
                           vlc_value_t oldval, vlc_value_t newval, void * );
static int EsDataCallback ( vlc_object_t *p_this, char const *psz_cmd,
                           vlc_value_t oldval, vlc_value_t newval, void * );
...
static const vlc_input_callback_t p_input_callbacks[] =
{
    ...
    CALLBACK( "data-es", EsDataCallback ),
...
void input_ControlVarInit ( input_thread_t *p_input )
{
    ...
    /* Data ES */
    var_Create( p_input, "data-es", VLC_VAR_INTEGER );
    var_Change( p_input, "data-es", VLC_VAR_SETVALUE, &val, NULL );
    text.psz_string = _("Data Track");
    var_Change( p_input, "data-es", VLC_VAR_SETTEXT, &text, NULL );
                            ...
                            vlc_value_t oldval, vlc_value_t newval, void *p_data )
{
    ...
    if( newval.i_int < 0) {
        newval.i_int = -DATA_ES;
    } else {
        var_SetBool( p_input, "data", true );
    }
