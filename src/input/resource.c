...
#include "../misc_output/misc_out_internal.h"
...
static misc_output_t *RequestMiscOut(input_resource_t *p_resource)
{
    vlc_assert_locked(&p_resource->lock);

    misc_output_t *p_misc = misc_out_New(p_resource->p_parent);
    p_resource->p_misc_out = p_misc;
    return p_misc;
}
...
/* */
misc_output_t *input_resource_RequestMiscOut(input_resource_t *p_resource)
{
    vlc_mutex_lock(&p_resource->lock);
    misc_output_t *p_ret = RequestMiscOut(p_resource);
    vlc_mutex_unlock(&p_resource->lock);
    return p_ret;
}

void input_resource_PutMiscOut(input_resource_t *p_resource, misc_output_t *p_misc)
{
    assert(p_misc != NULL);

    vlc_mutex_lock(&p_resource->lock_hold);
    if(p_misc == p_resource->p_misc_out) {
        p_misc = NULL;
    } else {
        msg_Dbg(p_resource->p_parent, "destroying misc output");
    }
    vlc_mutex_unlock(&p_resource->lock_hold);

    if(p_misc != NULL) {
        misc_out_Destroy(p_misc);
    }
}
